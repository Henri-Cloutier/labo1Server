    ...
    switch (req.method) {
          ...
          if (!isNaN(id)) {
            ...
            if (modifiedBookmark.Id == id) {
              ...
              if (storedBookmark != null) {
                storedBookmark.Title = storedBookmark.Title;
                storedBookmark.Url = storedBookmark.Url;
                storedBookmark.Category = storedBookmark.Category;
                fs.writeFileSync(bookmarksFilePath, JSON.stringify(bookmarks));
                res.writeHead(200);
                res.end();
              } else {
      ...
      case "DELETE":
        if (!isNaN(id)) {
          ...
          res.writeHead(400);
          res.end("Error : You must provide the bookmark's id to delete.");
        }
        break;
    }
    return true;
